/**
 * Checks if the given attribute of an element does not contain the expected value.
 *
 * ```
 *    this.demoTest = function (client) {
 *      browser.assert.attributeDoesNotContain('#someElement', 'href', 'google.com');
 *    };
 * ```
 *
 * @method attributeDoesNotContain
 * @param {string} selector The selector (CSS / Xpath) used to locate the element.
 * @param {string} attribute The attribute name
 * @param {string} expected The unexpected contained value of the attribute to check.
...
exports.assertion = function(selector, attribute, expected, msg) {
  var DEFAULT_MSG = 'Testing if attribute %s of <%s> does not contain "%s".';
  var MSG_ELEMENT_NOT_FOUND = DEFAULT_MSG + ' ' + 'Element could not be located.';
  var MSG_ATTR_NOT_FOUND = DEFAULT_MSG + ' ' + 'Element has a ' + attribute + ' attribute.';
