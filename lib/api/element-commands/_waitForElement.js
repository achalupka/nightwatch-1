...
WaitForElement.prototype.fail = function(result, actual, expected, defaultMsg) {
  ...
  return this.complete(result), this.client.api.getLog('browser', function(logEntriesArray) {
    if(logEntriesArray && Array.isArray(logEntriesArray)) {
      console.log(' [BROWSER_ERROR_LOG] ' + logEntriesArray.length + ' logs from browser found:');
